...
import { Upload, Mic, FileText, X, CheckCircle, ShieldAlert, Scissors, AlertTriangle, Save, RefreshCw, Square, User, Download } from 'lucide-react'; // ✅ Download importado
...
export const SurgicalReportView: React.FC<SurgicalReportViewProps> = ({ doctor, patient }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 p-4 md:p-8 overflow-y-auto animate-fade-in">
      ...
      {/* HEADER */}
      <div className="mb-6 flex justify-between items-end border-b dark:border-slate-800 pb-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <span className="bg-indigo-600 text-white p-1.5 rounded-lg"><Scissors size={20}/></span>
            Op-Scribe: Reporte Quirúrgico
          </h2>
          <p className="text-slate-500 text-sm mt-1">Generador de Notas Sinópticas Blindadas (NOM-004)</p>
        ...
        <div className="hidden md:flex items-center gap-2 text-xs text-amber-600 bg-amber-50 px-3 py-1 rounded-full border border-amber-100">
          <ShieldAlert size={14}/>
          <span>Modo Memoria Volátil Activo</span>
        </div>
      </div>
