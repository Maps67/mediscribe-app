...
interface SmartBriefingProps {
  ...
  isLoading?: boolean; // <--- AGREGADO: Propiedad vital para el Skeleton
  insights: {
...
const SmartBriefingWidget: React.FC<SmartBriefingProps> = ({ 
  ...
  systemStatus, 
  onOpenAssistant, 
  isLoading = false, // <--- Por defecto falso si no se pasa
  insights 
}) => {
  ...
  // --- SKELETON LOADER VISIBLE (CORRECCIÓN VISUAL) ---
  if (isLoading) {
    return (
      <div className="relative w-full rounded-[2.5rem] bg-slate-200 dark:bg-slate-800 p-8 mb-8 overflow-hidden h-[300px] border-2 border-slate-300 dark:border-slate-700 animate-pulse shadow-xl">
          {/* Fondo Shimmer sutil */}
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 dark:via-white/5 to-transparent skew-x-12 translate-x-[-100%] animate-[shimmer_1.5s_infinite]"></div>
          
          <div className="relative z-10 flex flex-col justify-end h-full w-full gap-6">
             {/* Simulación del Saludo */}
             <div className="space-y-3">
               <div className="h-4 w-32 bg-slate-400/50 dark:bg-slate-600 rounded-lg"></div>
               <div className="h-10 w-2/3 bg-slate-400/50 dark:bg-slate-600 rounded-2xl"></div>
             </div>
             
             {/* Simulación de las Tarjetas de Métricas */}
             <div className="flex gap-4">
                <div className="h-14 w-36 bg-slate-300 dark:bg-slate-700 rounded-2xl border border-slate-400/20"></div>
                <div className="h-14 w-36 bg-slate-300 dark:bg-slate-700 rounded-2xl border border-slate-400/20"></div>
                <div className="hidden sm:block h-14 w-36 bg-slate-300 dark:bg-slate-700 rounded-2xl border border-slate-400/20"></div>
             </div>
          </div>
      </div>
    );
