...
export const SubscriptionPlans = () => {
  ...
  // AJUSTE COMERCIAL: Precio actualizado a $1,199
  const prices = {
  ...
  const handleSubscribe = (plan) => {
    // Actualización del mensaje de WhatsApp con el nuevo precio
  ...
  return (
    // CORRECCIÓN DE UX (SCROLL & PADDING):
    // 1. 'p-0' en móvil: Elimina márgenes externos para usar el 100% del ancho.
    // 2. 'rounded-none' y 'shadow-none' en móvil: Elimina desperdicio de espacio en bordes.
    // 3. 'md:p-6': Mantiene la elegancia en escritorio.
    <div className="w-full max-w-4xl mx-auto bg-white md:rounded-2xl md:shadow-xl md:border border-slate-200 overflow-hidden">
      
      {/* Contenedor Interno con Padding Ajustado */}
      <div className="p-3 md:p-6">
        ...
        <div className="text-center mb-3 md:mb-10">
          ...
          <p className="text-slate-500 text-[11px] md:text-lg leading-tight mt-1">
            Suite clínica integral con IA.
          </p>
          
          {/* Toggle de Ciclo */}
          <div className="flex justify-center mt-3 md:mt-8">
            <div className="relative flex bg-slate-100 p-0.5 rounded-lg border border-slate-200">
              <button
                onClick={() => setBillingCycle('monthly')}
                className={`${
                  billingCycle === 'monthly' ? 'bg-white shadow-sm text-slate-900 font-bold' : 'text-slate-500 hover:text-slate-700 font-medium'
                } relative z-10 w-24 md:w-32 py-1.5 md:py-2 text-[10px] md:text-sm rounded-md transition-all duration-200`}
              >
                Mensual
              </button>
              <button
        ...
        <div className="grid md:grid-cols-2 gap-3 md:gap-8 items-start">
          ...
          {/* Padding interno reducido al mínimo legible (p-3) */}
          <div className="border-2 border-teal-500 rounded-xl p-3 md:p-8 relative bg-white shadow-lg ring-2 md:ring-4 ring-teal-50/50 z-10 isolate">
            ...
            <div className="flex justify-between items-center md:block mt-1 md:mt-0">
              ...
              {/* PRECIO MÓVIL (Compacto en Header) */}
              <div className="flex md:hidden items-baseline relative z-30 bg-white">
                  <span className="text-xl font-extrabold text-slate-900 tracking-tight">${billingCycle === 'monthly' ? '1,199' : '11.5k'}</span>
                  <span className="text-[10px] text-slate-400 font-medium ml-1">+IVA</span>
              </div>
            </div>
            
            {/* PRECIO ESCRITORIO (Completo) */}
            ...
            {/* Lista Ultra Compacta (text-[11px] para legibilidad) */}
            <ul className="space-y-1.5 md:space-y-4 mb-4 md:mb-8 relative z-20">
              <li className="flex items-start text-[11px] md:text-sm text-slate-700 font-medium">
            ...
            <button
              ...
              className="w-full py-2.5 md:py-3.5 px-4 bg-slate-900 text-white hover:bg-slate-800 font-bold rounded-lg md:rounded-xl transition-all shadow-md transform active:scale-95 flex items-center justify-center gap-2 relative z-20 text-xs md:text-base"
            >
              Comenzar Ahora
          ...
          <div className="border border-slate-200 bg-slate-50 rounded-xl p-3 md:p-8 relative mt-0 z-0 flex md:block justify-between items-center">
              <div>
                  ...
                  <div className="hidden md:block">
                    <p className="text-sm text-slate-400 mb-6">Control centralizado para equipos.</p>
        ...
        <p className="text-center text-[9px] md:text-xs text-slate-400 mt-3 md:mt-8 mx-auto max-w-lg">
