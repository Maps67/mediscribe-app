...
import { 
  ...
  Calendar, Pill, Stethoscope, Briefcase
} from 'lucide-react';
...
// --- GENERADOR DE vCARD (Tecnología Blob) ---
const generateVCard = (profile: UserProfile | null) => {
    ...
    link.href = url;
    link.setAttribute('download', `Dr_${profile.full_name.replace(/\s+/g, '_')}.vcf`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
};

// --- COMPONENTE: PERLA CLÍNICA DEL DÍA (NUEVO) ---
const ClinicalPearlCard = () => {
    const tips = [
        { title: "Nefroprotección", text: "En DM2, los iSGLT2 reducen progresión renal independientemente del control glucémico." },
        { title: "Hipertensión", text: "El monitoreo ambulatorio (MAPA) detecta HTA enmascarada en 15-30% de pacientes con lecturas normales en consultorio." },
        { title: "Antibióticos", text: "La duración corta de terapia antibiótica en neumonía adquirida en comunidad (5 días) es igual de efectiva que cursos largos." },
        { title: "Dislipidemia", text: "Añadir ezetimiba a estatinas reduce eventos CV adicionales un 6% vs monoterapia." }
    ];
    // Seleccionar tip basado en el día del año para rotación diaria consistente
        ...
        <div className="bg-gradient-to-r from-indigo-900 to-slate-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden mb-6 group hover:shadow-2xl transition-all duration-500">
            ...
            <div className="relative z-10">
                <div className="flex items-center gap-2 mb-3">
                    <div className="bg-indigo-500/20 p-1.5 rounded-lg backdrop-blur-sm border border-indigo-500/30 animate-pulse">
                ...
                <div className="mt-4 flex gap-2">
                    <span className="text-[10px] bg-white/10 px-2 py-1 rounded text-slate-400">Evidencia Nivel A</span>
                </div>
            </div>
        </div>
    );
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between aspect-square md:aspect-auto md:h-28 group hover:border-blue-200 transition-colors">
                    ...
                    <div><span className="text-2xl font-bold text-slate-800">{stats.patientsCount}</span><p className="text-xs text-slate-500 font-medium">Pacientes Activos</p></div>
                </div>
                {/* KPI 2 */}
                <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between aspect-square md:aspect-auto md:h-28 group hover:border-purple-200 transition-colors">
                    ...
                    <div><span className="text-2xl font-bold text-slate-800">{stats.avgDuration > 0 ? stats.avgDuration + 'm' : '--'}</span><p className="text-xs text-slate-500 font-medium">Tiempo/Consulta</p></div>
                </div>
                {/* Acceso Rápido 1 */}
            ...
            {/* Feed de Noticias (Estilo Tarjeta) */}
