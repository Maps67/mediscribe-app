  ...
  useEffect(() => {
    ...
    if (BrowserSpeechRecognition) {
      ...
      recognitionRef.current.onend = () => {
        if (isRecording) {
            try { recognitionRef.current.start(); } catch (e) { console.log("Reinicio automático"); }
        } else {
            setStatus('idle');
        }
      };
  ...
  const generateRecord = async () => {
    ...
    try {
      ...
      const newConsultation = await MedicalDataService.createConsultation({
        patient_id: '00000000-0000-0000-0000-000000000000', // ID Temporal hasta tener selector de pacientes
      ...
      const mockRecord: MedicalRecord = {
        ...
        objective: "Datos guardados exitosamente",
      ...
      console.error(e);
      alert("Error generando el expediente: " + (e instanceof Error ? e.message : "Error desconocido"));
    } finally {
               ...
               <div className="p-4 space-y-6">
                 <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                    <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                      ...
                      {generatedRecord && <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">Guardado en DB</span>}
                    </div>
