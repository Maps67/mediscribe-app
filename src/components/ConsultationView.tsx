...
import { Mic, Square, RefreshCw, Send, FileText, Stethoscope, ChevronDown, User, Search, Calendar, AlertTriangle, Beaker, Printer, Share2, History, Lock, Crown, Edit2, Eye, PenTool, X, Save, Image as ImageIcon, UploadCloud, Trash2, ExternalLink } from 'lucide-react';
...
const SPECIALTIES = ["Medicina General", "Cirujano General", "Cardiología", "Pediatría", "Ginecología y Obstetricia", "Traumatología y Ortopedia", "Medicina Interna", "Dermatología", "Psicología/Psiquiatría", "Nutrición", "Oftalmología", "Otorrinolaringología"];
...
const ConsultationView: React.FC = () => {
  ...
  const [doctorProfile, setDoctorProfile] = useState<DoctorProfile>({ full_name: 'Doctor', specialty: 'Medicina', license_number: '', phone: '', university: '', address: '', logo_url: '', signature_url: '', subscription_tier: 'basic' });
  ...
  const fetchDocuments = async (patientId: string) => {
      try {
        const { data: docs, error } = await supabase.from('patient_documents').select('*').eq('patient_id', patientId).order('created_at', { ascending: false });
        if (error) throw error;
        if (docs) {
            const docsWithUrls = await Promise.all(docs.map(async (doc) => {
                const { data } = await supabase.storage.from('patient-files').createSignedUrl(doc.file_url, 3600);
  ...
  const handleUploadFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        setNewDocName(''); fetchDocuments(selectedPatient.id); toast.success("Estudio subido correctamente.");
  ...
  const handleDeleteDocument = async (docId: string, filePath: string) => {
    ...
    try {
        if (filePath) await supabase.storage.from('patient-files').remove([filePath]);
      ...
      <div className="flex flex-col gap-4 shrink-0">
        <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
                {!isPro && <span className="text-[10px] bg-slate-200 text-slate-500 px-2 py-1 rounded font-bold border border-slate-300">BÁSICO</span>}
                ...
                <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${isListening ? 'bg-red-100 text-red-600 animate-pulse' : 'bg-slate-100 text-slate-500'}`}>{isListening ? '● Grabando' : '● En Espera'}</div>
            </div>
        </div>
            ...
            <div className="relative flex-1">
                ...
                {searchResults.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto z-50">
                        {searchResults.map(p => ( <div key={p.id} onClick={() => handleSelectPatient(p)} className="p-3 hover:bg-slate-50 cursor-pointer border-b border-slate-50 flex justify-between items-center"><div className="font-bold text-sm text-slate-700">{p.name}</div><ChevronDown className="-rotate-90 text-slate-300" size={16} /></div> ))}
                    </div>
                )}
            </div>
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-2 gap-6 flex-1 min-h-0 overflow-hidden">
        <div className={`bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden lg:h-full transition-all duration-300 ${generatedRecord ? 'h-[30vh]' : 'flex-1'}`}>
          ...
          <div className="p-4 border-t flex gap-3 bg-white shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            <button onClick={handleToggleRecording} disabled={!hasConsent} className={`flex-1 py-3 rounded-lg font-medium flex items-center justify-center gap-2 transition-all relative overflow-visible ${isListening ? 'bg-red-50 text-red-600 border border-red-200 mic-active' : 'bg-slate-900 text-white hover:bg-slate-800'}`}>
              {isListening ? <><Square size={18}/> Parar</> : <><Mic size={18}/> Iniciar</>}
            </button>
            <button onClick={generateRecord} disabled={!transcript || isListening || isLoadingRecord} className="px-4 py-3 bg-brand-teal text-white rounded-lg font-medium flex items-center gap-2 disabled:opacity-50 hover:bg-teal-600 transition-all shadow-lg shadow-teal-500/20">{isLoadingRecord ? <RefreshCw className="animate-spin" size={20}/> : <FileText size={20}/>} Generar</button>
          </div>
        </div>
        ...
        <div className={`bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden lg:h-full relative ${generatedRecord ? 'flex-1' : 'hidden lg:flex'}`}>
           <div className="flex border-b bg-slate-50 shrink-0 overflow-x-auto">
            <button onClick={() => setActiveTab('record')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap px-4 ${activeTab === 'record' ? 'bg-white text-brand-teal border-t-2 border-brand-teal' : 'text-slate-400'}`}>Expediente</button>
            <button onClick={() => setActiveTab('instructions')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap px-4 ${activeTab === 'instructions' ? 'bg-white text-brand-teal border-t-2 border-brand-teal' : 'text-slate-400'}`}>Paciente</button>
            <button onClick={() => setActiveTab('chat')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap px-4 ${activeTab === 'chat' ? 'bg-white text-brand-teal border-t-2 border-brand-teal' : 'text-slate-400'}`}>Chat IA</button>
          </div>
          ...
          <div className="flex-1 relative bg-white overflow-hidden">
             {activeTab === 'record' && ( <div className="absolute inset-0 flex flex-col"> {generatedRecord ? ( <> <div className="p-2 bg-slate-50 border-b border-slate-100 flex justify-between items-center shrink-0"> <span className="text-[10px] uppercase font-bold text-slate-400 ml-2">Nota Clínica SOAP</span> <button onClick={() => setIsEditingNote(!isEditingNote)} className={`flex items-center gap-2 px-3 py-1 rounded text-xs font-bold transition-colors ${isEditingNote ? 'bg-brand-teal text-white' : 'text-slate-500 hover:bg-slate-200'}`}> {isEditingNote ? 'Ver' : 'Editar'} </button> </div> <div className="flex-1 overflow-y-auto p-4"> {isEditingNote ? <textarea className="w-full h-full text-sm text-slate-700 outline-none resize-none font-mono leading-relaxed bg-transparent" value={editableSummary} onChange={(e) => setEditableSummary(e.target.value)} /> : <FormattedText content={editableSummary} />} </div> </> ) : ( <div className="flex-1 flex flex-col items-center justify-center text-slate-400"><FileText size={32} className="mb-2 opacity-20"/><p className="text-sm">Sin nota generada.</p></div> )} </div> )}
             {activeTab === 'instructions' && ( <div className="absolute inset-0 flex flex-col"> {generatedRecord ? ( <> <div className="flex-1 p-4 bg-green-50/30 overflow-y-auto"> <textarea className="w-full h-full bg-transparent outline-none resize-none text-sm text-slate-700 font-medium" value={patientInstructions} onChange={(e) => setPatientInstructions(e.target.value)} /> </div> <div className="p-3 border-t border-slate-100 flex flex-wrap justify-end items-center gap-2 bg-white shrink-0"> <button onClick={sendToWhatsApp} className={`px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors shadow-sm flex-1 justify-center md:flex-none ${isPro ? 'bg-[#25D366] text-white hover:bg-green-600' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}> {!isPro && <Lock size={12}/>} <Send size={14}/> WhatsApp </button> <button onClick={handleSharePDF} disabled={isSharing || !isPro} className={`px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors shadow-sm flex-1 justify-center md:flex-none ${isPro ? 'bg-brand-teal text-white hover:bg-teal-600' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}> {!isPro ? <Lock size={12}/> : (isSharing ? <RefreshCw size={14} className="animate-spin"/> : <Share2 size={14}/>)} <span>Compartir</span> </button> {isPro ? ( <PDFDownloadLink document={<PrescriptionPDF doctorName={doctorProfile.full_name} specialty={doctorProfile.specialty} license={doctorProfile.license_number} phone={doctorProfile.phone} university={doctorProfile.university} address={doctorProfile.address} logoUrl={doctorProfile.logo_url} signatureUrl={doctorProfile.signature_url} patientName={selectedPatient?.name || "Paciente"} date={new Date().toLocaleDateString()} content={patientInstructions} />} fileName={`Receta.pdf`} className="bg-slate-800 text-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-slate-700 transition-colors shadow-sm flex-1 justify-center md:flex-none"> {({ loading }) => (loading ? '...' : <><Printer size={14}/> <span className="hidden sm:inline">PDF</span></>)} </PDFDownloadLink> ) : ( <button className="bg-slate-200 text-slate-400 px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 cursor-not-allowed shadow-sm flex-1 justify-center md:flex-none"><Lock size={12}/> <Printer size={14}/> PDF</button> )} </div> </> ) : ( <div className="flex-1 flex flex-col items-center justify-center text-slate-400"><User size={32} className="mb-2 opacity-20"/><p className="text-sm">Sin instrucciones.</p></div> )} </div> )}
             {activeTab === 'chat' && ( <div className="absolute inset-0 flex flex-col bg-slate-50"> <div className="flex-1 p-4 overflow-y-auto space-y-3"> {chatMessages.map((m, i) => ( <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}> <div className={`p-3 rounded-lg text-sm max-w-[85%] shadow-sm ${m.role === 'user' ? 'bg-brand-teal text-white rounded-br-none' : 'bg-white text-slate-700 rounded-bl-none'}`}>{m.text}</div> </div> ))} <div ref={chatEndRef} /> </div> <div className="p-3 bg-white border-t border-slate-200 shrink-0"> <form onSubmit={handleAskAI} className="flex gap-2"> <input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Preguntar..." className="flex-1 p-2 border border-slate-200 rounded-lg text-sm outline-none" /> <button type="submit" disabled={!chatInput.trim()} className="bg-slate-900 text-white p-2.5 rounded-lg hover:bg-slate-800"><Send size={18}/></button> </form> </div> </div> )}
          </div>
        </div>
      </div>
                            ...
                            {documents.map(doc => (
                                <div key={doc.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden group relative shadow-sm cursor-pointer" onClick={() => doc.file_type === 'image' && setPreviewUrl(doc.signedUrl || '')}>
                                    <div className="aspect-square bg-slate-100 flex items-center justify-center relative overflow-hidden">
                                        {doc.file_type === 'image' ? ( <img src={doc.signedUrl} className="w-full h-full object-cover" onError={(e) => e.currentTarget.src = ''} /> ) : ( <div className="text-center"><FileText size={32} className="text-slate-400 mx-auto mb-1"/><span className="text-[10px] font-bold text-slate-500 uppercase">Documento</span></div> )}
                                        {!doc.signedUrl && <div className="absolute inset-0 flex items-center justify-center bg-slate-50"><RefreshCw className="animate-spin text-slate-300"/></div>}
                                    </div>
                                    <div className="p-2 text-xs font-bold truncate">{doc.name}</div>
                                    <button onClick={(e) => { e.stopPropagation(); handleDeleteDocument(doc.id, doc.file_url); }} className="absolute top-1 right-1 bg-white/90 p-1 rounded-full text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={12} /></button>
                                </div>
                            ))}
